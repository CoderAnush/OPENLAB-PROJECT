...
// Init Charts
const gaugeMQ2 = createGauge(document.getElementById('gaugeMQ2').getContext('2d'), 'MQ-2 Gas', '#8B5CF6', 2000, ' ppm');
const gaugeMQ135 = createGauge(document.getElementById('gaugeMQ135').getContext('2d'), 'MQ-135 Air', '#10B981', 2000, ' ppm');
...
function updateDashboard(data) {
    ...
    if (data.ml_probs) {
        document.getElementById('prob-safe').style.width = (data.ml_probs.safe * 100) + '%';
        document.getElementById('prob-warn').style.width = (data.ml_probs.warn * 100) + '%';
        document.getElementById('prob-crit').style.width = (data.ml_probs.crit * 100) + '%';
    }

    // Update Predictive Horizon
    const elTTC = document.getElementById('time-to-critical');
    const elContainer = document.getElementById('time-critical-container');

    if (elTTC && elContainer) {
        if (data.time_to_crit === 0) {
            elTTC.textContent = "CRITICAL CONDITION";
            elContainer.className = "flex items-center gap-2 p-2 rounded-lg bg-rose-600 border border-rose-700 text-white animate-pulse shadow-rose-500/50 shadow-lg";
        } else if (data.time_to_crit && data.time_to_crit !== "Stable" && !isNaN(parseFloat(data.time_to_crit))) {
            // If we have a critical prediction (e.g. 5.2)
            elTTC.textContent = `${data.time_to_crit}s to CRITICAL`;
            elContainer.className = "flex items-center gap-2 p-2 rounded-lg bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-900/50 text-rose-600 dark:text-rose-400 animate-pulse";
        } else if (data.time_to_warn === 0) {
            elTTC.textContent = "WARNING ACTIVE";
            elContainer.className = "flex items-center gap-2 p-2 rounded-lg bg-amber-500/10 border border-amber-500/50 text-amber-600 dark:text-amber-400";
        } else if (data.time_to_warn && !isNaN(parseFloat(data.time_to_warn))) {
            // If we have warning prediction
            elTTC.textContent = `${data.time_to_warn}s to Warning`;
            elContainer.className = "flex items-center gap-2 p-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/50 text-amber-600 dark:text-amber-400";
        } else {
            elTTC.textContent = "Stable Prediction";
            elContainer.className = "flex items-center gap-2 p-2 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/50 text-emerald-600 dark:text-emerald-400";
        }
    }
    ...
    // Convert Voltage to simulated PPM for display (Linear scaling: 1V ~= 350ppm)
    // This allows the user to see "gas units" while keeping the precision of voltage
    const ppmMQ2 = (data.mq2_voltage || 0) * 350;
    const ppmMQ135 = (data.mq135_voltage || 0) * 350;

    // Update Gauges with Dynamic Colors
    updateGauge(gaugeMQ2, Math.round(ppmMQ2), 2000, [600, 1000]);
    updateGauge(gaugeMQ135, Math.round(ppmMQ135), 2000, [600, 1000]);
